...
export default function App() {
  // --- STATE ---
  const [activeTab, setActiveTab] = useState("comparison"); // comparison, calculators, recommendation
  const [selectedProfile, setSelectedProfile] = useState("individual"); // individual, couple, family, senior
  const [selectedSumInsured, setSelectedSumInsured] = useState("15"); // 10, 15, 25, 50 Lakhs
  const [selectedZone, setSelectedZone] = useState("zone1"); // zone1 (tier 1), zone2 (tier 2), zone3 (tier 3)
  ...
  const [answers, setAnswers] = useState({
    PED: "no",
    maternity: "no",
    priority: "features", // features, budget, wellness
    ageGroup: "young" // young, family, senior
  });
  ...
  const proportionalDeductionResults = useMemo(() => {
    // If selected room is equal or cheaper, no deduction
    ...
    // Proportional formula: Admissible = Bill * (Limit / Actual)
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans antialiased">
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-8">
        ...
        {/* CONFIGURATOR WIDGET (Always visible at top of tabs for seamless premium changes) */}
        ...
        {activeTab === "comparison" && (
          <div>
            ...
            {/* DYNAMIC COMPARISON MATRIX GRID (THE SHEET STRUCTURE) */}
